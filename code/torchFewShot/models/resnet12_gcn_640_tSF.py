...
from .neck import ClassSpecific, TaskSpecific, FFN_MLP, tSF_stacker, tSF_encoder, SIQ_encoder, tSF_encoder_block
...
class ResNet(nn.Module):
    def __init__(self, block, layers, kernel=3, fpn=False, norm_layer=nn.BatchNorm2d):
        ...
        self.layer2 = self._make_layer(block, 160, layers[1], stride=2) # stride=1: out(21,21)
        self.layer3 = self._make_layer(block, 320, layers[2], stride=2)
        self.layer4 = self._make_layer(block, 640, layers[3], stride=2)

        self.tSF_layer3 = tSF_stacker(feature_dim=320, num_queries=5, num_heads=1, FFN_method='MLP', layer_num=1)
        self.tSF_layer4 = tSF_stacker(feature_dim=640, num_queries=5, num_heads=1, FFN_method='MLP', layer_num=1)
        self.conv_channel = nn.Conv2d(320, 640, kernel_size=1, bias=False)
        self.bn_channel = self.norm_layer(640)
        self.tSF_layer34 = tSF_encoder_block(feature_dim=640, num_heads=1, FFN_method='MLP')

        self.nFeat = 640 * block.expansion
    ...
    def forward(self, x):
        ...
        # tSF
        x3_tSF = self.tSF_layer3(x3)
        x4_tSF = self.tSF_layer4(x4)
        x3_tSF = self.conv_channel(x3_tSF)
        x3_tSF = self.bn_channel(x3_tSF)
        x3_tSF = self.relu(x3_tSF)
        x4 = self.tSF_layer34(x4_tSF, query_feat=x3_tSF)
...
def resnet12_gcn_640_tSF(norm_layer=nn.BatchNorm2d):
