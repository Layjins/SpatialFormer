...
# Transformer Spatial Class-Attention (TSCA) = SpatialFormer Class-Attention (SFCA)
# Transformer Class-Attention (TCA) = TransFormer Class-Attention (TFCA)
class TSCA(nn.Module):
    def __init__(self, feature_dim, num_heads=1, FFN_method='MLP', mode='TSCA', softmax_score=False):
...
# Transformer Spatial Instance-Attention (TSIA) = SpatialFormer Instance-Attention (SFIA)
# Transformer Instance-Attention (TIA) = TransFormer Instance-Attention (TFIA)
class TSIA(nn.Module):
    def __init__(self, feature_dim, num_heads=1, FFN_method='MLP', mode='TSIA'):
        super(TSIA, self).__init__()
        if mode == 'TSIA':
            self.tsca = TSCA(feature_dim, num_heads=num_heads, FFN_method=FFN_method, mode='TSCA', softmax_score=True)
        elif mode == 'TIA':
            self.tsca = TSCA(feature_dim, num_heads=num_heads, FFN_method=FFN_method, mode='TCA')
    ...
    def forward(self, support, query):
        ...
        - query.size = B, c, h, w. B=batch size
        """
        support = self.tsca(support, attention_feature=query)
        query = self.tsca(query, attention_feature=support)
